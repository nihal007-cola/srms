...
def get_grn_orders(db: Session = Depends(get_db)):
    ...
    for entry in rm_orders:
        ...
        extra_data = first_item.extra_data or {}
        ...
        status = extra_data.get('status', 'PROCESSED')
        if has_grn:
        ...
        result.append({
            'poToken': po_token,
            'supplier': extra_data.get('supplier', extra_data.get('supplierAlias', '')),
            'supplierAlias': extra_data.get('supplierAlias', extra_data.get('supplier', '')),
...
def save_grn(data: schemas.GRNSaveRequest, db: Session = Depends(get_db)):
    try:
        ...
        print(f"🔍 DEBUG: snapshot_updates count: {len(snapshot_updates)}")
        print(f"🔍 DEBUG: snapshot_updates: {snapshot_updates}")
...
def cancel_grn(data: Dict, db: Session = Depends(get_db)):
    try:
        ...
        if not remaining_grns:
            ...
            for rm in rm_orders:
                extra_data_copy = dict(rm.extra_data or {})
                extra_data_copy['status'] = 'PROCESSED'
                extra_data_copy['grnCancelledAt'] = datetime.utcnow().isoformat()
